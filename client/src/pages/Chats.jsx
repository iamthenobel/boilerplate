...
export default function Chats() {
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-2 md:px-4 flex gap-4 h-[80vh] transition-all duration-500 animate-fadeIn">
      {/* Left: Chat List */}
      <aside className="w-80 bg-white rounded-2xl shadow-md flex flex-col overflow-hidden transition-all duration-300 animate-slideInLeft">
        <h2 className="text-lg font-bold text-[#252525] px-6 py-4 border-b">Chats</h2>
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          {USERS.map(user => (
            <div
              key={user.id}
              className={`flex items-center gap-3 px-6 py-4 cursor-pointer transition-all duration-200 border-b last:border-b-0 hover:bg-indigo-50 ${selected.id === user.id ? 'bg-indigo-50' : ''}`}
      ...
      {/* Main: Chat Room */}
      <section className="flex-1 bg-white rounded-2xl shadow-md flex flex-col overflow-hidden transition-all duration-300 animate-slideInRight">
        {/* Chatroom Header */}
